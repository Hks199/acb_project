...
const getOrderDetails = async (req, res, next) => {
  try {
    ...
    const pipeline = [
      ...
      // Lookup variant combination correctly
      {
        $lookup: {
          from: "productvariantsets",
          let: { variant_combination_id: "$orderedItems.variant_combination_id" },
          pipeline: [
            {
              $unwind: "$combinations",
            },
            {
              $match: {
                $expr: { $eq: ["$combinations._id", "$$variant_combination_id"] },
              },
            },
            {
              $project: {
      ...
      {
        $addFields: {
          variant_combination: { $arrayElemAt: ["$variantSet.combination", 0] },
        },
      },
      { $project: { variantSet: 0 } },

      // Group back ordered items
      {
        $group: {
          _id: "$_id",
          user_id: { $first: "$user_id" },
